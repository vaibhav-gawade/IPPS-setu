...
import { CreditCard, ArrowUpRight, ArrowDownRight, Clock, CheckCircle } from 'lucide-react';

export default function PaymentsPage() {
  const [tab, setTab] = useState('All');

  const payments = [
    { id: 'PAY-8910', invoice: 'INV-2024-001', project: 'AI Drones - Milestone 1', amount: '₹50,00,000', date: '15 Sep 2024', status: 'Completed' },
    { id: 'PAY-8911', invoice: 'INV-2024-002', project: 'AI Drones - Milestone 2', amount: '₹50,00,000', date: 'Expected 15 Oct 2024', status: 'Pending' },
    { id: 'PAY-7822', invoice: 'INV-2023-014', project: 'Cybersecurity Audit Final', amount: '₹15,00,000', date: '10 Dec 2023', status: 'Completed' }
  ];
    ...
    <div className="page-enter">
      <div className="section-header">
        <div>
          ...
          <p className="section-subtitle">Track your invoices, pending payments, and payment history</p>
        </div>
      </div>

      <div style={{ display: 'flex', gap: '24px', marginBottom: '32px' }}>
        <div className="card" style={{ flex: 1 }}>
          <div style={{ fontSize: '14px', color: 'var(--text-muted)', marginBottom: '8px' }}>Total Received (YTD)</div>
          <div style={{ fontSize: '28px', fontWeight: 'bold', color: 'var(--green-400)' }}>₹65,00,000</div>
        </div>
        <div className="card" style={{ flex: 1 }}>
          <div style={{ fontSize: '14px', color: 'var(--text-muted)', marginBottom: '8px' }}>Pending Payments</div>
          <div style={{ fontSize: '28px', fontWeight: 'bold', color: 'var(--amber-400)' }}>₹50,00,000</div>
        </div>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          <thead>
            <tr style={{ background: 'var(--bg-card-hover)', borderBottom: '1px solid var(--border-color)' }}>
              ...
              <th style={{ padding: '16px', fontWeight: '500', color: 'var(--text-muted)' }}>Project / Invoice</th>
            ...
            {displayed.map(pay => (
              <tr key={pay.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                ...
                <td style={{ padding: '16px' }}>
                  <div style={{ fontWeight: '500' }}>{pay.project}</div>
...
// Note: Adding CheckCircle import since I missed it above in PaymentsPage
